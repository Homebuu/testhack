...
murderermystery2:Toggle({
    Title = "Anti Fling",
    Desc = "ป้องกันการถูก Fling",
    Value = false,
    Callback = function(state)
        _G.AntiFlingMM2 = state
        ...
        if not state then
                ...
                antiFlingConnection = game:GetService("RunService").Heartbeat:Connect(function()
                    ...
                    if linVel > 100 or angVel > 100 then
                        ...
                        game:GetService("StarterGui"):SetCore("SendNotification", {
                            Title = "Anti Fling",
                            Text = "ตรวจพบการ Fling! ป้องกันสำเร็จ",
                            Duration = 3
                        })
                    else
                    ...
                    for _, obj in pairs(hrp:GetChildren()) do
                        if obj:IsA("BodyVelocity") or 
                           ...
                           obj:IsA("BodyForce") then
                            if obj.Name ~= "FlyBodyVelocity" then
                                obj:Destroy()
                            end
                        end
                    end
            ...
            applyAntiFling(game.Players.LocalPlayer.Character)

            game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
                if _G.AntiFlingMM2 then
                    task.wait(0.5)
                    if antiFlingConnection then
                        antiFlingConnection:Disconnect()
                        antiFlingConnection = nil
